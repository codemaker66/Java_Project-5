...
public class OutputDaoImpl implements OutputDao {
	...
	public List<Person> retrievePersonByFirstAndLastName(String firstName, String lastName) {
		...
		for (int i = 0; i < Data.persons.size(); i++) {
			if (Data.persons.get(i).getFirstName().equals(firstName) && Data.persons.get(i).getLastName().equals(lastName)
				|| Data.persons.get(i).getFirstName().equals(firstName) && !Data.persons.get(i).getLastName().equals(lastName)) {
				list.add(Data.persons.get(i));
			}
		}
