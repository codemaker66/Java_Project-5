...
public class OutputService {
	...
	public List<Object> findPersonsByAddress(String address) {
		...
		if (persons.size() == 1) {
			fireStation
					.add("This person is served by the firestaion number : " + persons.get(0).getFireStationNumber());
		} else {
			fireStation.add(
	...
	public List<Object> findPersonsByFireStationNumbers(List<String> stations) {
		...
		Map<String, List<Person>> map = persons.stream().collect(Collectors.groupingBy(Person::getAddress));
		...
		for (List<Person> value : map.values()) {
			...
			if (value.size() == 1) {
				...
				string.add(
						"These persons are served by the firestation number : " + value.get(0).getFireStationNumber());
			}
			list.add(string);
			list.add(value);
		}
	...
	public List<Person> findEmailsByCity(String city) {
		...
		List<Person> list = output.retrievePersonsByCity(city);
